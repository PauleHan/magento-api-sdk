...
class CommandPool implements PromisorInterface
{
     ...
     *   The function is provided an TriggmineException object, id of the iterator that
            ...
            foreach ($commands as $key => $command) {
                if (!($command instanceof CommandInterface)) {
                    throw new \InvalidArgumentException('Each value yielded by '
                        . 'the iterator must be an Triggmine\CommandInterface.');
                }
                if ($before) {
                    $before($command, $key);
                }
     ...
     *
     * @return array
     * @see \Triggmine\CommandPool::__construct for available configuration options.
     */
