...
/**
 * "Waiters" are associated with an Triggmine resource (e.g., EC2 instance), and poll
...
{
    /** @var TriggmineClientInterface Client used to execute each attempt. */
    private $client;
     ...
     *
     * @param TriggmineClientInterface $client Client used to execute commands.
    ...
    private function matchesStatus($result, array $acceptor)
    {
        ...
        } elseif ($result instanceof TriggmineException && $response = $result->getResponse()) {
    ...
    {
        if ($result instanceof TriggmineException) {
            return $result->isConnectionError()
                || $result->getTriggmineErrorCode() == $acceptor['expected'];
        }
